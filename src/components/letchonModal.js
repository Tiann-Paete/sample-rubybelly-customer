...
const LetchonModal = ({ 
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      try {
        ...
        const filteredPrices = data
          .filter(product => type === 'belly' 
            ? product.name === 'Lechon Belly' 
            : product.name === 'Lechon Baboy')
          .map(product => ({
            ...
            quantity: product.quantity // Available quantity from database
          }));
        ...
        // Set default selected image to the first item
    ...
    if (isVisible) {
      ...
      setQuantities({}); // Reset quantities when modal closes
    }
  }, [isVisible, type]);
